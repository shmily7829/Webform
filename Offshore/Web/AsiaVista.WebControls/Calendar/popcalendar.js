...
//	email :	fuushikaden@yahoo.com
	...
	var	crossobj, crossMonthObj, crossYearObj, monthSelected, yearSelected, dateSelected, omonthSelected, oyearSelected, odateSelected, monthConstructed, yearConstructed, intervalID1, intervalID2, timeoutID1, timeoutID2,/* ctlToPlaceValue, ctlNow,*/ dateFormat, nStartingYear, sReturnDate
	sReturnDate = ""
    ...
    */
	...
	if (dom)
	{
		...
		document.write ("<div onclick='bShow=true' id='calendar'	class='div-style'><table width="+((showWeekNumber==1)?250:220)+" class='table-style'><tr class='title-background-style' ><td><table width='"+((showWeekNumber==1)?248:218)+"'><tr><td class='title-style'><B><span id='caption'></span></B></td><td align=right><a href='javascript:hideCalendar()' style='display: none'><IMG SRC='"+imgDir+"close.gif' WIDTH='15' HEIGHT='13' BORDER='0' ALT='Close the Calendar'></a></td></tr></table></td></tr><tr><td class='body-style'><span id='content'></span></td></tr>")
...
//	var	monthName =	new	Array("一　月", "二　月", "三　月", "四　月", "五　月", "六　月", "七　月", "八　月", "九　月", "十　月", "十一月", "十二月");
	if (startAt==0)
	{
...
//		dayName = new Array	("一","二","三","四","五","六","日");
	...
	function init()	{
		...
		if (!ns4)
		{
			...
			if (showToday==1)
			{
				//document.getElementById("lblToday").innerHTML =	todayString + " <span class='today-style' onmousemove='window.status=\""+gotoString+"\"' onmouseout='window.status=\"\"' title='"+gotoString+"' onclick='javascript:monthSelected=monthNow;yearSelected=yearNow;constructCalendar();'>" + yearNow + " 年 " + (monthNow+1) + " 月 " + dateNow + " 日　星期" + dayName[(today.getDay()-startAt==-1)?6:(today.getDay()-startAt)] + "</span>";
				document.getElementById("lblToday").innerHTML =	todayString + " <span class='today-style' onmousemove='window.status=\""+gotoString+"\"' onmouseout='window.status=\"\"' title='"+gotoString+"' onclick='javascript:monthSelected=monthNow;yearSelected=yearNow;constructCalendar();'>" + yearNow + "&nbsp;&nbsp;" + monthName[(monthNow)] + " " + dateNow + "&nbsp;&nbsp;" + dayName[(today.getDay()-startAt==-1)?6:(today.getDay()-startAt)] + "</span>";
			}
	...
	function closeCalendar() {
		...
		//hideCalendar();
		sReturnDate =	constructDate(dateSelected,monthSelected,yearSelected)
		//ctlToPlaceValue.value =	constructDate(dateSelected,monthSelected,yearSelected)

		window.returnValue.strDate = sReturnDate;
		window.close();
	}
	...
	function constructMonth() {
		...
		if (!monthConstructed) {
			...
			document.getElementById("selectMonth").innerHTML = "<table width=58	class='dropdown-style' cellspacing=0 onmouseover='clearTimeout(timeoutID1)'	onmouseout='clearTimeout(timeoutID1);timeoutID1=setTimeout(\"popDownMonth()\",100);event.cancelBubble=true'>" +	sHTML +	"</table>"
	...
	function popUpMonth() {
		...
		crossMonthObj.left = /*parseInt(crossobj.left) +*/ 60
	...
	function constructCalendar () {
		...
		for	( datePointer=1; datePointer<=numDaysInMonth; datePointer++ )
		{
			...
			else if	((dayPointer % 7 == (startAt * -1) +1) || (dayPointer % 7 == (startAt * -1) )) //end-of-the-week day(Sunday and Saturday)
			...
			sHTML += "<a class='"+sStyle+"' "+dateMessage+" title=\"" + sHint + "\" href='#' onclick='javascript:dateSelected="+datePointer+";closeCalendar();return false;'>&nbsp;" + datePointer + "&nbsp;</a>"
...
//		var c = document.all.calendar.firstChild;
		...
		if (bPageLoaded)
		{
			if ( crossobj.visibility ==	"hidden" ) {
				...
				if ( formatChar	!= "" )
				{
					...
					aData =	sReturnDate.split(formatChar)
